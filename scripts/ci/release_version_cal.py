...
def extract_module_version_info(mod_update_info, mod):
    """
    mod installed required in azdev
    azdev extension show --mod-name ${mod} --query pkg_version -o tsv
    """
    cmd = ["azdev", "extension", "show", "--mod-name", mod, "--query", "pkg_version", "-o", "tsv"]
    result = subprocess.run(cmd, stdout=subprocess.PIPE)
    if result.returncode != 0:
        print("error in get mod version")
        return
    mod_update_info["version_dev"] = result.stdout.decode("utf8").strip()
